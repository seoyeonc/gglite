...
make_df <- function(x, y = NULL) {
  ...
  if (!is.vector(y)) {
      ...
      dfy <- data.frame(y)
      df <- cbind(dfx,dfy)
      df <- tidyr::pivot_longer(df, cols = colnames(dfy), names_to = "tag", values_to = "y")
  } else {
...
make_args <- function(tag, args) {
  if (!is.null(tag) && any(names(args) %in% c("col", "color", "colour", "fill"))) {
    args[names(args) %in% c("col", "color", "colour", "fill")] <- NULL
    warning("When the tag option is used, the 'color' (or 'colour' or 'col') and 'fill' option will be ignored.")
  }
  return(args)
}
...
line <- function(x, y = NULL, tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(x, y)
  geom_type <- ggplot2::geom_line
  aes <- ggplot2::aes(x = x, y = y, col = tag)
  make_geom(df,geom_type,aes,args)
}

point <- function(x, y=NULL,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(x, y)
...
smooth <- function(x, y=NULL,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(x, y)
...
step <- function(x, y=NULL,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(x, y)
...
jitter <- function(x, y=NULL,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(x, y)
...
## 1d geoms
histogram <- function(y,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(y)
  geom_type <- ggplot2::geom_histogram
  aes <- ggplot2::aes(x = y, y = stat(density), fill = tag)
  args$alpha <- 0.5
  args$position <- "identity"
  make_geom(df,geom_type,aes,args)
}

density <- function(y,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(y)
  geom_type <- ggplot2::geom_density
  aes <- ggplot2::aes(x = y, fill = tag, col = tag)
  args$alpha <- 0.25
  make_geom(df,geom_type,aes,args)
}

qq <- function(y,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- data.frame(y)
  geom_type <- ggplot2::geom_qq
  aes <- ggplot2::aes(sample = y, col = tag)
  make_geom(df,geom_type,aes,args)
}

qq_line <- function(y,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- data.frame(y)
...
col <- function(x, y=NULL,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  df <- make_df(x, y)
  df$x <- as.factor(df$x)
  geom_type <- ggplot2::geom_col  
  aes <- ggplot2::aes(x = x, y = y, fill = tag)
  args$position <- 'dodge'
  make_geom(df,geom_type,aes,args)
}

boxplot <- function(x, y=NULL,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  if (is.null(y)) {
    y=x
    x=0
...
violin <- function(x, y=NULL,tag=NULL, ...) {
  args <- make_args(tag,list(...))
  if (is.null(y)) {
    y=x
    x=0
  ...
  aes <- ggplot2::aes(x = x, y = y, fill = tag, color = tag)
  args$alpha <- 0.5
  args$scale <- 'area'
  make_geom(df,geom_type,aes,args)
}